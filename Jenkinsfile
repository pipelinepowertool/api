...
//         pipelinePowerToolInitiator()
        configFileProvider([configFile(fileId: 'ce7257b3-97e2-4486-86ee-428f65c0ff26', variable: 'MAVEN_SETTINGS')]) {
          sh "mvn -s $MAVEN_SETTINGS -U package -Dquarkus.container-image.push=true -Dquarkus.container-image.username=${DOCKER_USER} -Dquarkus.container-image.password=${DOCKER_PASS}"
        }
      }
    }
  }
